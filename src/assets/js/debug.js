(function () {
    ...
    function getParentToolbarTheme() {
        ...
        try {
            ...
            root = window.frameElement.getRootNode ? window.frameElement.getRootNode() : null;
    ...
    function getUrlTheme() {
        try {
            return normalizeTheme(new URL(window.location.href).searchParams.get(themeParam));
        } catch (e) {
            return null;
        }
    }
    ...
    function addThemeToUrl(url, theme) {
        ...
        if (parsed.origin !== window.location.origin || parsed.pathname.indexOf('/debug/') === -1) {
            return url;
        }
    ...
    function applyTheme() {
        var theme = getUrlTheme()
            || normalizeTheme(document.documentElement.getAttribute('data-yii-debug-theme'))
        ...
        document.cookie = themeStorageKey + '=' + encodeURIComponent(theme) + '; path=/; SameSite=Lax';
    ...
    function preserveThemeInLinks(theme) {
        ...
        for (i = 0; i < forms.length; i++) {
            forms[i].setAttribute('action', addThemeToUrl(forms[i].getAttribute('action') || window.location.href, theme));
        ...
        while (current && current.nodeType === 1) {
            if (current.getAttribute) {
                ...
                if (current.getAttribute('data-toggle') === kind) {
                    if (!legacyToggleWarned && typeof console !== 'undefined' && console.warn) {
                        console.warn('[yii-debug] `data-toggle` is deprecated; use `data-yii-debug-toggle`.');
    ...
    function hideDropdowns(except) {
        ...
        for (var i = 0; i < wrappers.length; i++) {
            ...
            var trigger = wrappers[i].querySelector('[data-yii-debug-toggle="dropdown"], [data-toggle="dropdown"]');
            if (trigger) {
    ...
    function activateTab(link) {
        ...
        var links = list ? list.querySelectorAll('[data-yii-debug-toggle="tab"], [data-toggle="tab"]') : [];
        ...
        for (i = 0; i < panes.length; i++) {
            if (panes[i].classList && panes[i].classList.contains('yii-debug-tab-panel')) {
    ...
    document.addEventListener('click', function (event) {
        ...
        if (collapse) {
            var targetSelector = collapse.getAttribute('data-target') || collapse.getAttribute('href');
