(function () {
    ...
    function sameUrl(left, right) {
        ...
        return leftUrl !== null && rightUrl !== null && leftUrl.href === rightUrl.href;
    }
        ...
        XMLHttpRequest.prototype.open = function (method, url) {
            ...
            if (shouldTrackRequest(url)) {
                ...
                xhr.addEventListener('readystatechange', function () {
                    if (xhr.readyState === 4) {
                        item.duration = xhr.getResponseHeader('X-Debug-Duration') || new Date() - item.start;
                        item.loading = false;
        ...
        window.fetch = function (input, init) {
            ...
            if (shouldTrackRequest(url)) {
                ...
                promise.then(function (response) {
                    item.duration = response.headers.get('X-Debug-Duration') || new Date() - item.start;
                    item.loading = false;
    ...
    function YiiDebugToolbar() {
        ...
        self.activeUrl = '';
        self.expanded = window.localStorage && localStorage.getItem(storageKey) === '1';
    ...
    YiiDebugToolbar.prototype.disconnectedCallback = function () {
        ...
        if (this.systemThemeQuery) {
            if (this.systemThemeQuery.removeEventListener) {
                this.systemThemeQuery.removeEventListener('change', this.boundThemeRefresh);
    ...
    YiiDebugToolbar.prototype.detectTheme = function () {
        return normalizeTheme(this.getAttribute('data-yii-debug-theme'))
    ...
    YiiDebugToolbar.prototype.load = function () {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status !== 200) {
                self.renderError(xhr.responseText || 'Unable to load debug toolbar data.');
                return;
            }
    ...
    YiiDebugToolbar.prototype.renderError = function (message) {
        ...
        this.contentRoot.innerHTML = '<div class="toolbar expanded"><div class="bar"><strong>Yii Debugger</strong>'
            + '<span class="error-message">' + escapeHtml(message) + '</span></div></div>';
    };
    ...
    YiiDebugToolbar.prototype.render = function () {
        ...
        if (!this.data) {
            this.contentRoot.innerHTML = '<div class="toolbar loading"><button type="button" class="brand">Yii Debugger</button></div>';
            return;
        }

        var position = this.getAttribute('data-position') || this.data.position || 'bottom';
        ...
        var profilingPanel = (this.data.items || []).find(function (p) { return p && p.id === 'profiling'; });
        var profilingChip = profilingPanel ? this.renderPanel(profilingPanel) : '';

        this.contentRoot.innerHTML = '<div class="' + classes.join(' ') + '">'
            + '<div class="bar">'
            + (this.expanded
                ? this.renderBrand() + this.renderPhpChip() + profilingChip + this.renderAjaxPanel() + this.renderPanels(['profiling']) + this.renderControls()
    ...
    YiiDebugToolbar.prototype.renderCollapsedOpener = function () {
        ...
        return '<button type="button" class="brand brand-opener toggle-toolbar" title="Expand debug toolbar" aria-label="Expand debug toolbar">'
            + this.renderLogo() + '<span class="brand-text">' + title + '</span><span class="opener-icon">›</span></button>';
    };

    YiiDebugToolbar.prototype.renderBrand = function () {
        var configUrl = this.data.configUrl || this.data.indexUrl;
        var yiiAttr = configUrl ? ' data-debug-url="' + escapeHtml(this.withTheme(configUrl)) + '"' : '';
        var yiiVersion = this.data.yiiVersion
            ? '<span class="brand-version">' + escapeHtml(this.data.yiiVersion) + '</span>'
            : '';
        var yiiTitle = this.data.yiiVersion
            ? 'Yii ' + this.data.yiiVersion + ' — open configuration'
            : 'Open configuration';

        var yiiLink = '<span class="brand__link brand__link--yii" role="button" tabindex="0"' + yiiAttr + ' title="' + escapeHtml(yiiTitle) + '">'
            + this.renderLogo() + yiiVersion
            + '</span>';

        var phpLink = '';
        if (this.data.phpVersion) {
            var phpUrl = this.data.phpInfoUrl ? escapeHtml(this.withTheme(this.data.phpInfoUrl)) : '#';
            var phpTitle = 'PHP ' + this.data.phpVersion + ' — open phpinfo in a new tab';
            phpLink = '<a class="brand__link brand__link--php" href="' + phpUrl + '" target="_blank" rel="noopener" title="' + escapeHtml(phpTitle) + '">'
                + this.iconHtml('php-alt', 'panel-icon panel-icon--php')
                + '<span class="brand-version">' + escapeHtml(this.data.phpVersion) + '</span>'
                + '</a>';
        }

        var divider = phpLink ? '<span class="brand__divider" aria-hidden="true"></span>' : '';
    ...
    YiiDebugToolbar.prototype.renderAjaxPanel = function () {
        ...
        recent.forEach(function (request) {
            var requestStatus = request.loading ? 'loading' : request.error ? 'danger' : 'success';
            var profile = request.profilerUrl
                ? '<button type="button" class="ajax-link" data-debug-url="' + escapeHtml(request.profilerUrl) + '">' + escapeHtml(request.profile || 'profile') + '</button>'
                : 'n/a';

            rows += '<tr><td>' + escapeHtml(request.method || 'GET') + '</td>'
                + '<td><span class="badge badge-' + requestStatus + '">' + escapeHtml(request.statusCode || '-') + '</span></td>'
                + '<td class="ajax-url" title="' + escapeHtml(request.url) + '">' + escapeHtml(request.url) + '</td>'
                + '<td>' + escapeHtml(request.duration ? request.duration + ' ms' : '-') + '</td>'
                + '<td>' + profile + '</td></tr>';
        });

        if (rows === '') {
            rows = '<tr><td colspan="5" class="empty">No AJAX requests tracked yet.</td></tr>';
        }

        return '<div class="panel ajax-panel">'
            + icon
            + '<span class="panel-title">AJAX</span>'
            + '<span class="metric"><span class="metric-value badge-' + status + '">' + requests.length + '</span></span>'
    ...
    YiiDebugToolbar.prototype.iconHtml = function (iconName, cls) {
        ...
        return '<span class="' + cls + '" aria-hidden="true" style="-webkit-mask-image:url(' + escaped + ');mask-image:url(' + escaped + ')"></span>';
    };
    ...
    YiiDebugToolbar.prototype.renderPanel = function (panel) {
        ...
        var metrics = '';
        var items = panel.items || [];
        var rawTitle = typeof panel.title === 'string' ? panel.title : (panel.id || 'Panel');
        ...
        items.forEach(function (item) {
            ...
            var itemUrl = item.url ? ' data-debug-url="' + escapeHtml(item.url) + '"' : '';
            var itemTitle = item.title ? ' title="' + escapeHtml(item.title) + '"' : '';
            var metricClass = item.url && sameUrl(item.url, this.activeUrl) ? ' metric-active' : '';

            metrics += '<span class="metric' + metricClass + '"' + itemUrl + itemTitle + '>';
            if (item.icon) {
                metrics += self.iconHtml(item.icon, 'metric-icon');
            } else if (item.label) {
                metrics += '<span class="metric-label">' + escapeHtml(item.label) + '</span>';
            }
            metrics += '<span class="metric-value badge-' + escapeHtml(status) + '">' + escapeHtml(item.value) + '</span></span>';
        }, this);

        var iconHtml = panel.icon ? this.iconHtml(panel.icon, 'panel-icon') : '';

        return '<div class="panel' + panelClass + '" role="button" tabindex="0" title="' + escapeHtml(attrTitle) + '"' + url + '>'
            + iconHtml
            + (hasTitle ? '<span class="panel-title">' + escapeHtml(rawTitle) + '</span>' : '')
            + metrics + '</div>';
    };

    YiiDebugToolbar.prototype.renderControls = function () {
        var external = this.activeUrl
            ? '<a class="control" href="' + escapeHtml(this.withTheme(this.activeUrl)) + '" target="_blank" rel="noopener" title="Open panel in a new tab">↗</a>'
            : '<span class="control disabled" title="Open a panel first">↗</span>';
        var drawer = this.drawerOpen ? '<button type="button" class="control close-drawer" title="Close panel" aria-label="Close panel">×</button>' : '';
        var toggleTitle = this.expanded ? 'Collapse toolbar' : 'Expand toolbar';
        var toggleText = this.expanded ? '›' : '‹';

        return '<div class="controls">' + external + drawer
            + '<button type="button" class="control toggle-toolbar" title="' + toggleTitle + '" aria-label="' + toggleTitle + '">' + toggleText + '</button></div>';
    };
    ...
    YiiDebugToolbar.prototype.renderDrawer = function (position) {
        ...
        var handle = '<div class="resize-handle" title="Resize debug panel" aria-label="Resize debug panel"></div>';
        var drawer = '<div class="drawer"><iframe src="' + escapeHtml(this.withTheme(this.activeUrl)) + '" title="Yii debug panel"></iframe></div>';
    ...
    YiiDebugToolbar.prototype.bindDelegatedEvents = function () {
        ...
        root.addEventListener('click', function (event) {
            var target = closest(event.target, '[data-debug-url]') || closest(event.target, '.legacy-panel a[href]');
            var url = target ? target.getAttribute('data-debug-url') || target.getAttribute('href') : null;
    ...
    YiiDebugToolbar.prototype.applyDrawerHeight = function () {
        ...
        if (!this.style.getPropertyValue('--yii-debug-toolbar-drawer-height')) {
            var height = parseInt(this.getAttribute('data-height') || this.data.defaultHeight || 50, 10);
            this.style.setProperty('--yii-debug-toolbar-drawer-height', Math.max(20, Math.min(90, height)) + 'vh');
        }
    };
    ...
    YiiDebugToolbar.prototype.onPointerMove = function (event) {
        ...
        var height = drawerRect === null
            ? position === 'upper' ? event.clientY : viewportHeight - event.clientY
            : position === 'upper' ? event.clientY - drawerRect.top : drawerRect.bottom - event.clientY;

        this.style.setProperty('--yii-debug-toolbar-drawer-height', Math.max(120, Math.min(viewportHeight - 48, height)) + 'px');
    };
