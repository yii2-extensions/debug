 ...
 *
 * Centralizes the address splitting (`to`, `cc`, `bcc`, `reply`), the scalar-to-string coercion for header/body fields,
...
final class MailMessageNormalizer
{
     ...
     *
     * @param mixed $data Raw model supplied by GridView/ListView.
     */
     ...
     * (for example, SwiftMime / Symfony Mailer address objects), so a plain `is_string` narrow would drop legitimate
     * values.
     *
    ...
    /**
     * Splits a comma-separated address list into a `list<string>`, trimming surrounding whitespace and dropping empty
     * entries.
     *
     * @return list<string> Addresses in declaration order, possibly empty.
     */
    ...
    /**
     * Converts the captured time field into a Unix-epoch int.
     *
     * Accepts {@see DateTimeInterface}, ints, and `strtotime()`-parseable strings; everything else collapses to `null`.
     *
     * @param mixed $value Raw time entry as captured by the mailer.
     */
