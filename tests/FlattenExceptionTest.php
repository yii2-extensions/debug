...
final class FlattenExceptionTest extends TestCase
{
    ...
    public function testFlattenedArgumentsAreNormalizedByType(): void
    {
        ...
        self::assertIsResource(
            $dh,
            "'opendir()' must hand back an iterable directory stream for the fixture path.",
        );
        self::assertIsResource(
            $fh,
            "'tmpfile()' must hand back a usable temporary stream.",
        );
        ...
        $get = static function (int $position) use ($argsList): array {
            self::assertArrayHasKey(
                $position,
                $argsList,
                "Argument list must expose slot {$position}.",
            );
        ...
        self::assertSame(
            ['object', 'stdClass'],
            $get(0),
            "stdClass instances must be tagged 'object'.",
        );
        self::assertSame(
            ['object', 'yii\web\NotFoundHttpException'],
            $get(1),
            'Yii exception type name must be retained.',
        );
        self::assertSame(
            ['incomplete-object', 'BogusTestClass'],
            $get(2),
            '__PHP_Incomplete_Class must be tagged separately.',
        );
        self::assertSame(
            ['resource', 'stream'],
            $get(3),
            'Stream resources must be tagged with their type name.',
        );
        self::assertSame(
            ['resource', 'stream'],
            $get(4),
            'Temporary file streams must be tagged the same as opendir streams.',
        );
        ...
        self::assertSame(
            'object',
            $closureArgs[0],
            "Closures must be tagged 'object'.",
        );
        ...
        self::assertIsString(
            $closureClass,
            'Closure tag payload must surface as the class name string.',
        );
        self::assertTrue(
        ...
        self::assertSame(
            ...
            $get(6),
            'Numeric arrays must keep value tags per element.',
        );
        self::assertSame(
            ['array', ['foo' => ['integer', 123]]],
            $get(7),
            'Associative arrays must preserve string keys.',
        );
        self::assertSame(
            ['null', null],
            $get(8),
            "'null' must be tagged with a null payload.",
        );
        self::assertSame(
            ['boolean', true],
            $get(9),
            "'true' must be tagged 'boolean'.",
        );
        self::assertSame(
            ['boolean', false],
            $get(10),
            "'false' must be tagged 'boolean'.",
        );
        self::assertSame(
            ['integer', 0],
            $get(11),
            "Integer '0' must be preserved as integer.",
        );
        self::assertSame(
            ['float', 0.0],
            $get(12),
            "Float '0.0' must be preserved as float.",
        );
        self::assertSame(
            ['string', '0'],
            $get(13),
            "String '0' must be preserved as string.",
        );
        self::assertSame(
            ['string', ''],
            $get(14),
            'Empty string must be preserved as string.',
        );
        self::assertSame(
            ['float', INF],
            $get(15),
            'Infinity must round-trip as float.',
        );
        ...
        self::assertSame(
            'float',
            $nanEntry[0],
            'NaN must still be tagged float.',
        );
        self::assertNan(
            $nanEntry[1],
            'NaN payload must remain a NaN value.',
        );
    }
    ...
    public function testGetCodeMirrorsOriginalException(): void
    {
        ...
        self::assertSame(
            100,
            $flattened->getCode(),
            'Code must round-trip from the original exception.',
        );
    }
    ...
    public function testGetFileMatchesThrowSiteFile(): void
    {
        ...
        self::assertSame(
            __FILE__,
            $flattened->getFile(),
            'File must point to the throw site of the source exception.',
        );
    }
    ...
    public function testGetLineMatchesThrowSiteLine(): void
    {
        ...
        self::assertSame(
            $throwLine,
            $flattened->getLine(),
            'Line must point to the throw site of the source exception.',
        );
    }
    ...
    public function testGetMessageMirrorsOriginalException(): void
    {
        ...
        self::assertSame(
            ...
            'Message must round-trip from the original exception.',
        );
    }
    ...
    public function testGetPreviousReturnsFlattenedChain(): void
    {
        ...
        self::assertInstanceOf(
            ...
            $previous,
            "'getPrevious' must yield a flattened chain link.",
        );
        self::assertSame(
            $flattened2->getTrace(),
            $previous->getTrace(),
    ...
    public function testGetTraceCapturesNamespaceClassAndFunction(): void
    {
        ...
        self::assertNotEmpty(
            $trace,
            'Flattened trace must include the throw-site frame.',
        );
        ...
        self::assertSame(
            ...
            $frame['short_class'],
            'Short class name must drop the namespace.',
        );
        self::assertSame(
            __FUNCTION__,
            $frame['function'],
            'Trace frame must carry the throw-site method name.',
        );
    }
    ...
    public function testOversizedArgumentArraysAreTruncated(): void
    {
        ...
        self::assertArrayHasKey(
            0,
            $trace[0]['args'],
            'First flattened argument must be present.',
        );
        self::assertSame(
    ...
    private function extractArgsList(FlattenException $flattened): array
    {
        ...
        self::assertNotEmpty(
            $trace,
            'FlattenException must capture at least one stack frame.',
        );
        ...
        self::assertArrayHasKey(
            0,
            $args,
            'Outer tuple list must expose the first argument slot.',
        );
        ...
        self::assertIsArray(
            $outer,
            'Outer tuple slot must be a tagged tuple, not the SKIPPED sentinel.',
        );
        ...
        self::assertIsArray(
            $payload,
            'Tagged array payload must be a list of tagged tuples.',
        );
        ...
        foreach ($payload as $index => $entry) {
            self::assertIsInt(
                $index,
                'Inner argument list must be numerically indexed.',
            );
            self::assertIsArray(
                $entry,
                'Each inner argument must be a tagged tuple.',
            );
            self::assertArrayHasKey(
                0,
                $entry,
                "Tagged tuple must declare a 'type' slot.",
            );
            self::assertArrayHasKey(
                1,
                $entry,
                "Tagged tuple must declare a 'payload' slot.",
            );
            ...
            self::assertIsString(
                $type,
                "Tagged tuple 'type' slot must be a string discriminator.",
            );
